...
export async function GET(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    const session = await BrowserAssistantService.getSession(params.sessionId, payload.userId);
    ...
    console.error("Unhandled Get Session Error:", error);
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch assistant session" } },
      { status: 500 }
    );
  }
}
